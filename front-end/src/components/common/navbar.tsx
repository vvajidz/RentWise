...
export default function Homenavbar() {
  ...
  return (
    <>
      {showDesktopNav && (
        <nav className={`${philosopher.className} hidden md:block fixed top-0 bg-white z-50 w-full transition-transform duration-500`}>
          <div className="container mx-auto flex items-center justify-between px-6 py-4">
            <Link href="/" className="text-3xl font-bold text-gray-900 tracking-wide transition-transform hover:scale-105">RentWise</Link>
      ...
      <div className="block md:hidden py-4 text-center w-full">
        <Link href="/" className={`${philosopher.className} text-3xl font-bold text-gray-900 tracking-wide transition-transform hover:scale-105`}>
          RentWise
        </Link>
      </div>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 md:hidden bg-gray-900 text-white rounded-full p-3 shadow-lg z-50"
      ...
      >
        ...
        {user ? (
          <>
            <Link href="/dashboard" onClick={() => setMenuOpen(false)} className="block px-4 py-2 rounded-md bg-gray-900 text-white hover:bg-gray-700 text-base transition-colors shadow">Dashboard</Link>
            <Link href="/profile" onClick={() => setMenuOpen(false)} className="block px-4 py-2 rounded-md border border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white text-base transition-colors shadow">Profile</Link>
            <button
              onClick={() => {
                logoutUser();
                setMenuOpen(false);
